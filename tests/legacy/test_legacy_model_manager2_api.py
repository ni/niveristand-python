...
def test_model_manager2_legacy():
    ...
    try:
        ...
        CONST_VALUES = [
            CONST_1by5_initValue,
            CONST_2by3_initValue,
            CONST_5by1_initValue,
        ]
        ...
        with pytest.raises(NIVeriStandException):
            mmgr.SetSingleParameterValue(
                "NON_EXISTING_CONTROLLER", nonVectorParamList[1], 2
            )

        for param in nonVectorParamList:
            print(("Verifying Set Paramater " + param))
            result = mmgr.GetSingleParameterValue("Controller", param)
            assert result == 2, "Error verifying set single parameter value %s %d" % (
                param,
                result,
            )
        ...
        with pytest.raises(NIVeriStandException):
            mmgr.SetMultipleParameterValues(
                "NON_EXISTING_CONTROLLER", nonVectorParamList, newValue
            )
        ...
        for i in range(0, 6):
            ...
            mmgr.SetParameterVectorValues(
                "Controller", vectorParamList[i], CONST_MODVALUES[i]
            )
            time.sleep(1)
        ...
        TESTVALUES = [
            SINGLE_PARAM_TESTVALUE,
            CONST_1by5_TestValue,
        ...
        assert (
            mmgr.GetParameterVectorValues("Controller", "CONST1by5")
            == CONST_1by5_TestValue
        ...
        assert (os.path.isfile(SAVE_STATE_LOC)) == 1, (
            "Error verifying that the save parameter state file is on disk %s"
            % SAVE_STATE_LOC
        )
        ...
        assert (
            timeAtSave <= timeRestored <= currentTime
        ), "Error verifying restore parameter state"
