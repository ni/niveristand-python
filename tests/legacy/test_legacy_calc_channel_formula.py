...
def test_calculated_channel_formula_legacy():
    ...
    try:
        ...
        ), "Time-Time (%g) does not match calculated (%g)" % (expectedResult, result)
        print("...Pass")
        ...
        assert (
            result == expectedResult
        ), "Zero-1 (%g) does not match calculated (%g)" % (expectedResult, result)
        print("...Pass")
        ...
        ), "T-acos(cos(T)) (%g) does not match calculated (%g)" % (
            expectedResult,
            result,
        )
        print("...Pass")
        ...
        assert (
            expectedResult - tol <= result <= expectedResult + tol
        ), "T-asin(sin(T)) (%g) does not match calculated (%g)" % (
            expectedResult,
            result,
        )
        print("...Pass")
        ...
        assert (
            expectedResult - tol <= result <= expectedResult + tol
        ), "Sin^2(T)+Cos^2(T) (%g) does not match calculated (%g)" % (
            expectedResult,
            result,
        )
        print("...Pass")
        ...
        assert (
            result == expectedResult
        ), "Test value (%g) does not match calculated (%g)" % (expectedResult, result)
        print("...Pass")
        ...
        results = workspace.GetMultipleChannelValues(channels)
        print(
            "...Got mean=%g, variance=%g, errorInVariance=%g"
        ...
        assert (
            expectedResult - tol <= result <= expectedResult + tol
        ), "Integral of (FilterOut - 4) does not match expected. Error is %g " % (
            result
        )
        print("...Pass")
        ...
        print(
            "Checking final calculated channels time equals initial calculated channels time"
        )
