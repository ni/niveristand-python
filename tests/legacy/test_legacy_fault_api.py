...
def test_fault_api_legacy():
    ...
    system_definition = os.path.join(
        os.getcwd(),
        r"tests\testutilities\legacy_files\FaultChannelTest\FaultChannelTest.nivssdf",
    )
    ...
    try:
        ...
        assert (
            faultedValue == 512
        ), "Channel is faulted and not returning expected value"
        ...
        assert (
            checkValues == chanValues
        ), "Get Channel Value differ from Get Fault List result"
