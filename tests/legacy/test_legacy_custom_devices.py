...
def _test_device(workspace, base_name, multiplier):
    ...
    for x in a:
        ...
        assert abs(value_out - (multiplier * x + 1000)) > 0.1, (
...
@pytest.mark.skip(
    "currently failing, and does not appear to exercise anything special in the legacy API, so we will leave this skipped"
)
def test_custom_devices_legacy():
    ...
    system_definition = os.path.join(
        os.getcwd(),
        r"tests\testutilities\legacy_files\CustomDevices\CustomDevices.nivssdf",
    )
