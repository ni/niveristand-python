...
from NationalInstruments.VeriStand.RealTimeSequenceDefinitionApiUtilities import (
    CompilerUtilities,
)
...
def compile_rtseq(rtseq):
    success, _, compilation_events = CompilerUtilities.TryGetCompiledInstance(
        rtseq, False, [], []
    )
    if not success:
        errormsg = " ".join(
